...
war1 = Warehouse(1, 40, 30, 25) # возможно как-то имеет смысл сделать уникальное присвоение номера для склада,
...
while True:
    ...
    for i in my_w:
        if (int(fromW) == i.get_nubmer()):
            list_eq = i.get_equipments(name_equipment, int(amount))
